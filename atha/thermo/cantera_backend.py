...
class CanteraBackend(ThermoBackend):
    ...
    def combustion_equilibrium(
        ...
        P_chamber: float,   # Pa
        T_reactants: float = 300.0,  # K; use actual propellant temperature for cryogenic accuracy
    ) -> CombustionResult:
        """
        ...
        T_reactants: initial temperature of reactants (K). Default 300 K is appropriate
                     for ambient-temperature propellants; use ~90 K for LOX, ~20 K for LH2.
        """
        gas = self._gas
        Y_fuel = 1.0 / (1.0 + MR)
        Y_ox   = MR  / (1.0 + MR)
        gas.Y = {fuel: Y_fuel, oxidizer: Y_ox}
        gas.TP = T_reactants, P_chamber
        gas.equilibrate('HP')

        g = gas.cp_mass / gas.cv_mass
        # ct.gas_constant is in J/(kmol·K), mean_molecular_weight in kg/kmol → ratio = J/(kg·K)
        ...
        return CombustionResult(
            T_ad=float(gas.T),
            gamma=float(g),
            MW=float(gas.mean_molecular_weight) / 1000.0,   # convert kg/kmol → kg/mol
    ...
    def _to_fluid_state(self) -> FluidState:
        ...
        return FluidState(
            ...
            MW=float(g.mean_molecular_weight) / 1000.0,   # convert kg/kmol → kg/mol
