...
rpm = np.linspace(15000.0, 33000.0, len(t))
...
for i in range(len(t)):
    out_l = lox_pump.compute_outputs(t[i], {}, {"shaft.omega": omega[i], "inlet.P": P_lox_tank, "inlet.h": h_lox, "inlet.mdot": mdot_lox})
    out_f = fuel_pump.compute_outputs(t[i], {}, {"shaft.omega": omega[i], "inlet.P": P_fuel_tank, "inlet.h": h_fuel, "inlet.mdot": mdot_fuel})
...
ax[0].plot(rpm / 1000, lox_dp / 1e6, label="LOX pump")
ax[0].plot(rpm / 1000, fuel_dp / 1e6, label="Fuel pump")
ax[0].set(xlabel="Speed [krpm]", ylabel="delta_P [MPa]", title="Pump pressure rise vs speed")
ax[0].grid(alpha=0.3)
ax[0].legend()

ax[1].plot(rpm / 1000, lox_tau, label="LOX pump")
ax[1].plot(rpm / 1000, fuel_tau, label="Fuel pump")
ax[1].set(xlabel="Speed [krpm]", ylabel="tau_load [N*m]", title="Pump shaft load vs speed")
ax[1].grid(alpha=0.3)
ax[1].legend()

finish_plot("Example 10 Component Check: Pump speed ramp", "02_pumps_speed_ramp.png")
